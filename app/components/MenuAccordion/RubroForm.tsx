    ...
    <div className="flex justify-between">

      {
        ...
        && (
          ...
          <form onSubmit={handleSubmit} className="flex-1 flex justify-between absolute z-10 left-0 right-0 top-[95%] bg-white">
            ...
            <div className="relative flex-1 flex flex-col gap-1 justify-end items-end p-2">
              <div className="min-h-[290px] min-w-[220px] relative">
                <input onClick={() => dateRef.current?.showPicker()} className="absolute top-[10%] left-[25%] w-[10ch] text-center bg-red-100 rounded-lg" ref={showDateRef} readOnly value={actualDate} />
                <input className="w-0 absolute -top-[8%]" ref={dateRef} type="date" name="date" onChange={handleCalendarChange} />
              </div>
              <input className="border border-slate-500 rounded-lg text-center bg-red-100" type="number" name="monto" placeholder="monto" onFocus={(e) => e.currentTarget.select()} onChange={(e) => setActualMonto(Number(e.currentTarget.value))} value={actualMonto} />
              <button type="submit" className={`border border-slate-600 p-4 py-2 rounded-lg ${isLoading && "opacity-50"}`} disabled={isLoading}>{isLoading ? "Cargando" : "Agregar"}</button>
            </div>
  ...
  return (
    <div className="flex-1 flex flex-col static z-10 py-2">
      {sectores.map(sector =>
        ...
        <span
          className={`mx-1 px-1 rounded-lg border border-transparent hover:border hover:border-slate-400 ${actualSector === sector ? "bg-red-100 text-black" : "text-slate-400"}`}
          key={sector}
